...
func (gs *Store) Set(key, value []byte) {
	...
	const freeTier = 1024 // bytes
	if totalLen > freeTier {
		excess := totalLen - freeTier
		penaltyPerByte := 300
		extraGas := uint64(excess * penaltyPerByte)
		gs.gasMeter.ConsumeGas(extraGas, "custom write penalty over 1KB")
	}
